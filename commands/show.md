...
1. **Check for exact slug match:**
   ```bash
   ls .claude/prds/*[input]*.md 2>/dev/null
   ```
...
1. **Search using grep:**
   ```bash
   grep -r -i -n "<keyword>" .claude/prds --include="*.md" --exclude-dir=templates
   ```
...
File: .claude/prds/android-optimize-loginviewmodel.md
